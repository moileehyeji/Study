...
a = Conv2D(filters = 64,kernel_size=(12,12), strides=(1,1),padding='same',kernel_regularizer=regularizers.l2(1e-5)) (a)
...
mc = ModelCheckpoint('C:/Study/lotte/data/h5/31_effi_cnn.h5',save_best_only=True, verbose=1)
...
# predict
model.load_weights('C:/Study/lotte/data/h5/31_effi_cnn.h5')
result = model.predict(x_pred,verbose=True)

sub = pd.read_csv('C:/Study/lotte/data/sample.csv')
sub['prediction'] = np.argmax(result,axis = 1)
sub.to_csv('C:/Study/lotte/data/31.csv',index=False)
...
for i in tqdm(range(tta_steps)):
   # generator 초기화
    ...
    sub = pd.read_csv('C:/Study/lotte/data/sample.csv')
    sub['prediction'] = np.argmax(result,axis = 1)
    sub.to_csv('C:/Study/lotte/data/csv/31_tta_i.csv',index=False)

pred = np.mean(predictions, axis=0)

sub = pd.read_csv('C:/Study/lotte/data/sample.csv')
sub['prediction'] = np.argmax(result,axis = 1)
sub.to_csv('C:/Study/lotte/data/31_tta.csv',index=False) """
