...
mc = ModelCheckpoint('C:/Study/lotte/data/h5/28_EfficientNetB2.h5',save_best_only=True, verbose=1)
...
lr = ReduceLROnPlateau(patience= 4, factor=0.4)

model.compile(loss='categorical_crossentropy', optimizer=SGD(lr=0.1, momentum=0.9), metrics=['acc'])
learning_history = model.fit_generator(train_generator,epochs=50, steps_per_epoch= len(x_train) / 64,
    validation_data=valid_generator, callbacks=[early_stopping,lr,mc])

# predict
model.load_weights('C:/Study/lotte/data/h5/28_EfficientNetB2.h5')
...
sub.to_csv('C:/Study/lotte/data/28.csv',index=False)
