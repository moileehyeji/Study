...
eff = VGG19(weights="imagenet", include_top=False, input_shape=x_train.shape[1:])
eff.trainable = True
b = eff.output
...
a = Dense(2024, activation= 'relu') (top_model)
# a = Dropout(0.2) (a)
...
merge1 = Dense(1000, activation="softmax")(merge1)
model = Model(inputs=[EfficientNetB4.input, mobile_net.input], outputs = merge1)

'''


#=========================================컴파일, 훈련
mc = ModelCheckpoint('C:/Study/lotte/data/h5/11_ensemble2.h5',save_best_only=True, verbose=1)
...
#=========================================predict
model.load_weights('C:/Study/lotte/data/h5/11_ensemble2.h5')
...
sub.to_csv('C:/Study/lotte/data/11.csv',index=False)
