...
#==============================================kfold
skf = StratifiedKFold(n_splits=2, shuffle=True, random_state=66)
...
for train, valid in skf.split(x,y):
    ...
    mc = ModelCheckpoint(f'C:/Study/lotte/data/h5/19_EfficientNetB2_kfold{i}.h5',save_best_only=True, verbose=1)
    ...
    learning_history = model.fit_generator(train_generator,epochs=1, steps_per_epoch= len(x_train) / 16,
        validation_data=valid_generator, callbacks=[early_stopping,lr,mc])

    # predict
    model.load_weights(f'C:/Study/lotte/data/h5/19_EfficientNetB2_kfold{i}.h5')
    ...
    sub.to_csv(f'C:/Study/lotte/data/19_{i}.csv',index=False)
