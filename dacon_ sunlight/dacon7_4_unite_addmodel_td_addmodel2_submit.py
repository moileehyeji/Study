...
# 함수 : Td, T-Td(이슬점, 습수) column 추가
def Add_features_td(data):
    c = 243.12
    b = 17.62
...
def mymodel1():
    ...
    model.add(Dense(520, activation = 'relu',input_dim = 10))
...
def preprocess_data(data, is_train=True):
    ...
    data = Add_features_ghi(data)
    data = Add_features_td(data)
    data = data.astype('float64')
    temp = data.copy()
    temp = temp[['TARGET','Hour_Minute','GHI','Td', 'T-Td', 'DHI','DNI','WS','RH','T']]
...
for q in quantiles : 
        ...
        opti = Adam(learning_rate=0.0009)
        model.compile(loss=lambda y,pred: quantile_loss(q, y, pred), optimizer=opti , metrics='mae')
        ...
        submission.to_csv(f'./dacon/submission/submission0125_td_addmodel2/submission_7_1_{q}.csv', index = False)  
...
# to csv
submission.to_csv('./dacon/submission/submission0125_td_addmodel2/submission_7_2.csv', index = False)  # score :1.9498123625
...
'''
1. Quantile 분위수 늘리기 전:
loss_mean :  2.008448282877604


2. Quantile 분위수 늘린 후:
1)  lr : 0.1, fac : 0.3 / lr : 0.09, fac : 0.5 
loss_mean :  2.1384795109430947

3. td컬럼 추가 후:  -> 모델과 lr, fac 변경
loss_mean :  1.9686302145322163

4. 훈련 무조건 2번 :
loss_mean :  1.9431294070349798
