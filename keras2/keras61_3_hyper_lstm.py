...
# 2. 모델
# 하이퍼 파라미터 튜닝히여 반환한 변수이름 주의
def build_model(drop=0.5,optimizer=Adam,learning_rate=0.1):
...
# 하이퍼 파라미터 튜닝
# batch_size의 경우 기본값이 있으므로 실 사용시 기재 안해도 된다.
def create_hyperparameter() :       
    batchs = [10, 20, 30, 40, 50]
    optimizers = [RMSprop, Adam, Adadelta]
    dropout = [0.1, 0.2, 0.3]
    learning_rate = [0.1, 0.005, 0.001]
    return {'batch_size' : batchs, 'optimizer' : optimizers, 'drop':dropout, 'learning_rate' : learning_rate}   
