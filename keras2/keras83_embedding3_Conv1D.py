...
model = Sequential()
model.add(Embedding(input_dim=28,   #어휘의 크기 + 1(=word_size 단어사전의 갯수)
                                    #훈련위해 데이터가 들어가면 갯수가 중요해짐 그전엔 no의미
                                    #숫자가 같거나 커야함
                    output_dim=11,  #다음 layer로 넘어가는 노드의 수(임의로 넣어주자)
                                    #벡터화했을때의 크기
                    input_length=5))#시퀀스의 최대 길이(데이터의 column수)/  Flatten다음 Dense층  / 중요한 파라미터는 아니다 WHY? 넣고 안넣고 파라미터수 같음 
                    # (None, 5, 11) / parameter:308
# model.add(Embedding(27,11)) #(None, None, 11) / parameter:308 / 다음 Flatten층 안먹힘
# model.add(Flatten())    #(None, 11)을 인식하지 못하므로 전 Embedding 파라미터 input_length추가 해줘야함
# model.add(Dense(1))
# model.add(LSTM(32))
