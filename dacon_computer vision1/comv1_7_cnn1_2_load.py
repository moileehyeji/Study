...
x_train = x_train.reshape(-1, 28, 1, 28)
...
# 2. 모델구성
model = Sequential()
model.add(Conv2D(filters = 32, kernel_size=2, input_shape =(x_train.shape[1],x_train.shape[2],x_train.shape[3]), strides=2, padding='same'))
...
# 3. 컴파일, 훈련
opti = [ 'Nadam']       #, 'Adam', 'Adadelta', 'Adamax', 'Adagrad', 'RMSprop', 'SGD', 'Nadam']
loss_list = []
...
#     model.fit(x_train, y_train, epochs=1000, batch_size=30, validation_split=0.2 ,callbacks=[early, lr, cp])
...
model.load_weights('./dacon/computer/modelcheckpoint/comv1_7_cnn_28_0.3750.hdf5')
...
submit.to_csv('./dacon/computer/comv1_7_cnn.csv',index=False)
...
로드웨이트 : [1.9783966541290283, 0.36097562313079834]
'''
