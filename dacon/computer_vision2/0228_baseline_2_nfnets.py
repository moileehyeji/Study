...
#   model = nn.DataParallel(model)
    ...
    for epoch in range(epochs):
        ...
        if np.mean(valid_batch_accuracy)>valid_best_accuracy:
            torch.save(model.state_dict(), 'C:/Study/dacon/computer2/data/csv/EfficientNetB3-nfnets-fold{}.pt'.format(fold))
...
with torch.no_grad():
    for fold in range(1):
        model = EfficientNet_MultiLabel(in_channels=3).to(device)
        model.load_state_dict(torch.load('C:/Study/dacon/computer2/data/csv/EfficientNetB3-nfnets-fold2.pt'.format(fold)))
        model.eval()
...
submission.to_csv('C:/Study/dacon/computer2/data/csv/EfficientNetB3-nfnets-fold3.csv', index=False)  

# fold : 1        epoch : 02      train_accuracy / loss : 0.50251 / 49.74875      valid_accuracy / loss : 0.50238 / 49.76192   time : 717