...
device = torch.device(
    "cuda:0" if torch.cuda.is_available() else "cpu") # 디바이스 설정

# 3. Dataset 구성
dirty_mnist_answer = pd.read_csv("./dacon/computer2/data/dirty_mnist_2nd_answer.csv")
# dirty_mnist라는 디렉터리 속에 들어있는 파일들의 이름을 
# namelist라는 변수에 저장
namelist = os.listdir('./dacon/computer2/data/dirty_mnist_2nd/')
...
for fold_index, (trn_idx, val_idx) in enumerate(kfold.split(dirty_mnist_answer),1):
    ...
    #Dataset 정의
    train_dataset = DatasetMNIST("./dacon/computer2/data/dirty_mnist_2nd/", train_answer)
    valid_dataset = DatasetMNIST("./dacon/computer2/data/dirty_mnist_2nd/", test_answer)
...
# 7. 앙상블 적용
#test Dataset 정의
sample_submission = pd.read_csv("./dacon/computer2/data/sample_submission.csv")
test_dataset = DatasetMNIST("./dacon/computer2/data/test_dirty_mnist_2nd/", sample_submission)
batch_size = 128
...
predictions_list = []
# 배치 단위로 추론
prediction_df = pd.read_csv("./dacon/computer2/data/sample_submission.csv")
...
sample_submission.to_csv("./dacon/computer2/data/baseline_prediction.csv", index = False)