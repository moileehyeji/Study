...
imgs_dir = np.array(sorted(glob.glob('C:/Study/dacon/computer2/data/mnist2_img/train/*')))[:]
labels = np.array(labels_df.values[:,1:])
test_imgs_dir = np.array(sorted(glob.glob('C:/Study/dacon/computer2/data/mnist2_img/test/*')))
...
#   model = nn.DataParallel(model)
    ...
    for epoch in range(epochs):
        ...
        if np.mean(valid_batch_accuracy)>valid_best_accuracy:
            torch.save(model.state_dict(), 'C:/Study/dacon/computer2/data/csv/0228_prepro_baseline_2_fold{}.pt'.format(fold))
...
with torch.no_grad():
    for fold in range(1):
        model = EfficientNet_MultiLabel(in_channels=3).to(device)
        model.load_state_dict(torch.load('C:/Study/dacon/computer2/data/csv/0228_prepro_baseline_2_fold2.pt'.format(fold)))
        model.eval()
...
submission.to_csv('C:/Study/dacon/computer2/data/csv/0228_prepro_baseline_2_fold2.csv', index=False)  


#b3/ 5fold / 8batch / EfficientNetB3-fold0.csv
#b3/ 3fold / 8batch / EfficientNetB3-fold0(2).csv
# fold : 1        epoch : 32      train_accuracy / loss : 0.92580 / 0.19009       valid_accuracy / loss : 0.89189 / 0.28381       time : 788
# fold : 2        epoch : 32      train_accuracy / loss : 0.91922 / 0.20519       valid_accuracy / loss : 0.88939 / 0.28238       time : 786
# fold : 3        epoch : 32      train_accuracy / loss : 0.93722 / 0.16334       valid_accuracy / loss : 0.89978 / 0.26880       time : 787
# acc,loss 계속 오르고 낮아짐
