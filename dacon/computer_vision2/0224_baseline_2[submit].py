...
with torch.no_grad():
    for fold in range(1):
        model = EfficientNet_MultiLabel(in_channels=3).to(device)
        model.load_state_dict(torch.load('C:/Study/dacon/computer2/data/csv/EfficientNetB3-fold0.pt'.format(fold)))
        model.eval()
...
submission.to_csv('C:/Study/dacon/computer2/data/csv/EfficientNetB3-fold0.csv', index=False)  
