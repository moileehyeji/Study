...
# Dacon Dataset Load
labels_df = pd.read_csv('./dacon/computer2/data/dirty_mnist_2nd_answer.csv')[:]
imgs_dir = np.array(sorted(glob.glob('./dacon/computer2/data/dirty_mnist_2nd/*')))[:]
labels = np.array(labels_df.values[:,1:])
test_imgs_dir = np.array(sorted(glob.glob('./dacon/computer2/data/test_dirty_mnist_2nd/*')))
...
#   model = nn.DataParallel(model)
    ...
    for epoch in range(epochs):
        ...
        if np.mean(valid_batch_accuracy)>valid_best_accuracy:
            torch.save(model.state_dict(), './dacon/computer2/data/csv/EfficientNetB0-fold{}.pt'.format(fold))
...
# Test 추론
submission = pd.read_csv('./dacon/computer2/data/sample_submission.csv')

with torch.no_grad():
    for fold in range(1):
        model = EfficientNet_MultiLabel(in_channels=3).to(device)
        model.load_state_dict(torch.load('./dacon/computer2/data/csv/EfficientNetB0-fold{}.pt'.format(fold)))
        model.eval()
...
submission.to_csv('./dacon/computer2/data/csv/EfficientNetB0-fold0.csv', index=False)        