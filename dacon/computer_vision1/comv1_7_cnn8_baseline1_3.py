...
# x = np.where((x<=20)&(x!=0) ,0.,x)      #0 < x_train <=20 0 으로 바꿔
# x = x/255.
# x = x.astype('float32')
...
x_pred = np.where((x_pred<=20)&(x_pred!=0) ,0.,x_pred)
x_pred = x_pred/255
...
# 데이터 전처리
x_train, x_test, y_train, y_test = train_test_split(x, y, test_size = 0.03, shuffle = True, random_state=104)
x_train, x_val, y_train, y_val = train_test_split(x_train, y_train, test_size = 0.1, shuffle = True, random_state=104)
...
# train_generator = idg.flow(x_train,y_train,batch_size=batch_size)
...
for train_idx, test_idx in kfold.split(x):
    ...
    lr = ReduceLROnPlateau(monitor='val_acc', patience=5, factor=0.3, verbose=1) 
    modelpath = './dacon/computer/modelcheckpoint/comv1_7_cnn8_baseline1_3_{epoch:02d}_{val_acc:.4f}.hdf5'
    ...
    for i in opti:
        ...
        learning_history=model.fit(train_generator, epochs=1000,validation_data=val_generator , callbacks=[early, lr, cp])
    ...
    model.save('./dacon/computer/h5/comv1_7_cnn8_baseline1_3.h5')
    ...
    # print(y_pred)
    # print(y_pred.shape)
    result += model.y_pred/batch_size


# submit
submit['digit'] = np.argmax(result, axis=1)
submit.to_csv('./dacon/computer/comv1_7_cnn8_baseline1_2.csv',index=False)
...
[[0.34506825 0.91219515]Nadam
 ...
 [[0.30583775 0.91262138]]

 [[0.27822292 0.91935486]]
'''