...
#--------------Conv2D
w1 = tf.compat.v1.get_variable('w1', shape = [3,3,1,n1])
...
for epoch in range(training_epochs):
    ...
    print('Epoch : ', '%4d' %(epoch + 1), 
          'cost  :{:.9f}'.format(avg_cost)) #Epoch :    15 cost  :0.134050575

print('훈련 끝')

prediction = tf.equal(tf.argmax(hypothesis, 1), tf.argmax(y,1))
accuracy = tf.reduce_mean(tf.cast(prediction, tf.float32))
print('acc : ', sess.run(accuracy, feed_dict={x:x_test, y:y_test})) #dropout추가 : 0.19/ 제거: acc :  0.9622
