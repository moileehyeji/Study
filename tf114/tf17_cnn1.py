...
# bias는 기본값이 있을 것
# w1의 shape = [3,3,1,32]
# ->shape = [kernel_size[0],kernel_size[1],input(channel),output(filters)
# L1의 strides = [1,1,1,1]
...
#--------------Conv2D
w1 = tf.get_variable('w1', shape = [3,3,1,n1])
...
print(L1)
w2 = tf.get_variable('w2', shape = [3,3,n1,n2])
...
print(L2)
w3= tf.get_variable('w3', shape = [3,3,n2,n3])
...
print(L3)
w4= tf.get_variable('w4', shape = [3,3,n3,n4])
...
print(L_flat) 
#--------------Dense
w5 = tf.get_variable('w5', shape = [2*2*n4,n5], initializer = initializer1)
b5 = tf.Variable(tf.random_normal([n5], name = 'b1'))
L5 = tf.nn.selu(tf.matmul(L_flat,w5) + b5)
# L5 = tf.nn.dropout(L5, keep_prob=0.2) #shape=(?, 64)
print(L5)
w6 = tf.get_variable('w6', shape = [n5,n6], initializer = initializer1)
b6 = tf.Variable(tf.random_normal([n6], name = 'b2'))
L6 = tf.nn.selu(tf.matmul(L5,w6) + b6)
# L6 = tf.nn.dropout(L6, keep_prob=0.2) #shape=(?, 32)
print(L6)
#--------------Output
w7 = tf.get_variable('w7', shape = [n6,10], initializer = initializer1)
...
for epoch in range(training_epochs):
    ...
    print('Epoch : ', '%4d' %(epoch + 1), 
          'cost  :{:.9f}'.format(avg_cost))  #Epoch :    15 cost  :3.051944032

print('훈련 끝')

prediction = tf.equal(tf.argmax(hypothesis, 1), tf.argmax(y,1))
accuracy = tf.reduce_mean(tf.cast(prediction, tf.float32))
print('acc : ', sess.run(accuracy, feed_dict={x:x_test, y:y_test})) #dropout추가 : 0.19/ 제거: acc :  0.958
