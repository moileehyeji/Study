...
model.fit(x_train, y_train, eval_metric='logloss', eval_set=[(x_train, y_train), (x_test, y_test)])
eval_metric : model.compile의 metrics
eval_set    : validation
'''
...
# GridSearchCV : 이 자체가 모델
model = GridSearchCV(XGBClassifier(use_label_encoder=False), parameters, cv=kfold)       #(모델, 파라미터, kfold)   
