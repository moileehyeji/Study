...
# 데이터 열기 
x_train, x_test, y_train, y_test = np.load("./project/data/npy/modeling_xy_test.npy", allow_pickle = True)
...
# 학습 완료된 모델 저장
hdf5_file = path + "/h5/modeling_xy_test.hdf5"
if os.path.exists(hdf5_file):
    # 기존에 학습된 모델 불러들이기
    model.load_weights(hdf5_file)
else:
    # 학습한 모델이 없으면 파일로 저장
    model.fit(x_train, y_train, epochs=100, batch_size=10, callbacks=[er, lr], validation_split = 0.2)
...
# 적용해볼 이미지 
test_image = './project/data/img/test3.jpg'
# 이미지 resize
...
# loss= 2.698176145553589
# accuracy= 0.5929791331291199
# New data category :  gymball

# New data category :  dumbbell X reformer
